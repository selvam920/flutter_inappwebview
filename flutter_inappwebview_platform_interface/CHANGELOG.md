...
- Documented `PlatformCookieManager.flush` container semantics: on Android it flushes the default jar and fans out across every container created via `InAppWebViewSettings.containerId`; recommended as an on-pause lifecycle hook since cookies are committed to disk lazily
